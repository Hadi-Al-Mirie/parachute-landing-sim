class ParachuteSimulator {
  ...
  initScene() {
    ...
    this.plane.position.set(-100, 1500, 0);
  ...
  startPhysicsDisplay() {
    setInterval(() => {
      ...
      const planePos = `${p.x.toFixed(1)}, ${p.y.toFixed(1)}, ${p.z.toFixed(
        1,
      )}`;
      ...
      const playerPos = `${q.x.toFixed(1)}, ${q.y.toFixed(1)}, ${q.z.toFixed(
        1,
      )}`;
  ...
  animate() {
    ...
    if (this.isSimulationRunning) {
      ...
      if (!this.hasLanded) {
        ...
        this.camera.position.lerp(landedPos.clone().add(cameraOffset), 0.02);
